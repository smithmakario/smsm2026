<x-admin-layout>
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="flex items-center justify-between sticky top-0 z-10 bg-white/80 dark:bg-[#111a22]/80 backdrop-blur-md border-b border-slate-200 dark:border-[#243647] px-8 py-3">
            ...
            <a href="{{ route('admin.users.show', $user) }}" class="text-slate-600 dark:text-[#93adc8] hover:text-primary text-sm font-medium">View</a>
        </header>
        <div class="p-8 max-w-2xl">
            ...
            @endif
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6 rounded-xl bg-white dark:bg-[#111a22] border border-slate-200 dark:border-[#344d65] p-6 shadow-sm">
                @csrf
                @method('PUT')
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-[#243647]">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="marital_status" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Marital Status</label>
                        ...
                        <div>
                            <label for="occupation_category" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Occupation Category</label>
                            <select id="occupation_category" name="occupation_category"
                                ...
                                @foreach ($occupationCategories ?? [] as $value => $label)
                                    <option value="{{ $value }}" {{ old('occupation_category', $user->occupation_category) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('occupation_category')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="occupation" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Occupation</label>
                            <input id="occupation" name="occupation" type="text" value="{{ old('occupation', $user->occupation) }}"
                                class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary"
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-[#243647]">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="church" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Church</label>
                            <input id="church" name="church" type="text" value="{{ old('church', $user->church) }}"
                                class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary"
                                placeholder="Church name">
                            @error('church')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="country" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Country</label>
                            <input id="country" name="country" type="text" value="{{ old('country', $user->country ?? 'Nigeria') }}"
                                class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">
                            @error('country')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="state" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">State</label>
                            ...
                            <div>
                                <label for="city" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">City</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-[#243647]">
                    <div>
                        <label for="password" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">New Password (leave blank to keep)</label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Confirm New Password</label>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.users.show', $user) }}" class="rounded-lg bg-slate-200 dark:bg-[#243647] px-4 py-2 text-slate-700 dark:text-white text-sm font-bold hover:bg-slate-300 dark:hover:bg-[#344d65] transition-all">
                        Cancel
                    </a>
                </div>
            </form>
