    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="flex items-center justify-between sticky top-0 z-10 bg-white/80 dark:bg-[#111a22]/80 backdrop-blur-md border-b border-slate-200 dark:border-[#243647] px-8 py-3">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.events.show', $event) }}" class="text-slate-500 dark:text-[#93adc8] hover:text-primary">
                    <span class="material-symbols-outlined">arrow_back</span>
                </a>
                <h2 class="text-lg font-bold tracking-tight">Edit {{ $event->title }}</h2>
            </div>
        </header>
        <div class="p-8 max-w-2xl">
            ...
            @endif
            <form method="POST" action="{{ route('admin.events.update', $event) }}" class="space-y-6 rounded-xl bg-white dark:bg-[#111a22] border border-slate-200 dark:border-[#344d65] p-6 shadow-sm">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">{{ old('description', $event->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="format" name="format" required class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white focus:ring-primary focus:border-primary">
                            <option value="onsite" {{ old('format', $event->format) === 'onsite' ? 'selected' : '' }}>Onsite</option>
                            <option value="virtual" {{ old('format', $event->format) === 'virtual' ? 'selected' : '' }}>Virtual</option>
                        </select>
                    </div>
                </div>
                ...
                <div id="location-field">
                    ...
                    <input id="location" name="location" type="text" value="{{ old('location', $event->location) }}" placeholder="Venue address"
                        class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">
                </div>
                ...
                <div id="meeting-link-field" class="hidden">
                    ...
                    <input id="meeting_link" name="meeting_link" type="url" value="{{ old('meeting_link', $event->meeting_link) }}" placeholder="https://meet.google.com/..."
                        class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">
                </div>
                ...
                <div id="price-field" class="hidden">
                    ...
                    <input id="price" name="price" type="number" step="0.01" min="0" value="{{ old('price', $event->price) }}" placeholder="0.00"
                        class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">
                </div>

                <div>
                    <label for="capacity" class="block text-sm font-medium text-slate-700 dark:text-[#93adc8] mb-1">Capacity (optional)</label>
                    <input id="capacity" name="capacity" type="number" min="1" value="{{ old('capacity', $event->capacity) }}" placeholder="No limit"
                        class="w-full rounded-lg border-slate-300 dark:border-[#344d65] dark:bg-[#243647] dark:text-white shadow-sm focus:ring-primary focus:border-primary">
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.events.show', $event) }}" class="rounded-lg bg-slate-200 dark:bg-[#243647] px-4 py-2 text-slate-700 dark:text-white text-sm font-bold hover:bg-slate-300 dark:hover:bg-[#344d65] transition-all">
                        Cancel
                    </a>
                </div>
            </form>
