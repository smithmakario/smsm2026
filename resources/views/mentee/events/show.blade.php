...
@section('title', $event->title . ' – Mentee Portal')

@php $activeNav = 'events'; @endphp

@section('content')
<div class="space-y-6 max-w-3xl">
    <a href="{{ route($area . '.events.index') }}" class="inline-flex items-center gap-1 text-slate-500 dark:text-slate-400 hover:text-primary text-sm">
    ...
    <div class="bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
        ...
        @if($event->description)
            <p class="text-slate-600 dark:text-slate-400 text-sm mb-4 whitespace-pre-wrap">{{ $event->description }}</p>
        @endif
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div>
                <dt class="text-slate-500 dark:text-slate-400 font-medium">Date & time</dt>
            ...
            <div>
                <dt class="text-slate-500 dark:text-slate-400 font-medium">Type</dt>
            ...
            <div>
                <dt class="text-slate-500 dark:text-slate-400 font-medium">Format</dt>
                <dd class="text-slate-900 dark:text-white font-medium">{{ ucfirst($event->format) }}</dd>
            </div>
            <div>
                <dt class="text-slate-500 dark:text-slate-400 font-medium">{{ $event->format === 'virtual' ? 'Meeting link' : 'Location' }}</dt>
        ...
        <div class="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-wrap gap-4 items-center">
            @if($registration)
                ...
                <form action="{{ route($area . '.events.unregister', $event) }}" method="POST" class="inline" onsubmit="return confirm('Unregister from this event?');">
                    @csrf
                    <button type="submit" class="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-400 rounded-lg text-sm font-bold hover:bg-slate-300 dark:hover:bg-slate-700 transition-all">Unregister</button>
                </form>
                <div class="flex-1 min-w-0">
                    <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Share guest registration link</label>
                    <div class="flex gap-2">
                        <input type="text" readonly value="{{ $guestRegisterUrl }}" class="flex-1 min-w-0 rounded-lg border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white text-sm px-3 py-2">
            ...
            @else
                ...
                @endif
                <p class="text-slate-500 dark:text-slate-400 text-xs">After registering, you can share a link for guests to register.</p>
            @endif
        </div>
    </div>
</div>
